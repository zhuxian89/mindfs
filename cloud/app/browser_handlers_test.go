...
func TestBrowserAuthStatusRequiresAdminSession(t *testing.T) {
	...
	sessionToken := browserAdminSession(t, application)
	request := httptest.NewRequest(http.MethodGet, "/api/auth/me", nil)
	request.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
	...
	if payload["id"] != "usr_bootstrap" || payload["username"] != "admin" {
...
func TestBrowserLoginUsesSafeRelayTargets(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "non relay path", next: "/bind", want: "/nodes"},
...
func TestBrowserRecoveryRoutesRequireLogin(t *testing.T) {
	...
	if login.Code != http.StatusOK || !strings.Contains(login.Body.String(), `id="bootstrap-login"`) {
	...
	for _, contract := range []string{"/n/", "MindFSLauncherNodeSync", "This node is offline.", "setInterval(load,15000)"} {
...
func TestBrowserLogoutClearsSessionCookie(t *testing.T) {
	application := newBrowserTestApp(t)
	recorder := httptest.NewRecorder()
	application.Handler().ServeHTTP(recorder, httptest.NewRequest(http.MethodPost, "/api/auth/logout", nil))
...
func TestRelayNodesListRequiresSessionAndReturnsClientPayload(t *testing.T) {
	...
	emptyRequest.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
...
func TestRelayNodeRenameRequiresSameOriginAndPersists(t *testing.T) {
	application := newBrowserTestApp(t)
	now := time.Date(2026, 8, 4, 11, 0, 0, 0, time.UTC)
	createRelayTestNode(t, application, store.Node{ID: "node-rename", DeviceID: "device-rename", Name: "Before", Status: "active", AccessMode: "node_auth", CreatedAt: now}, []byte("rename-code"))
	sessionToken := browserAdminSession(t, application)
	unauthorized := httptest.NewRecorder()
	...
	empty.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
	...
	missing.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
...
func TestRelayNodeDeleteRevokesTokenAndDisconnects(t *testing.T) {
	application := newBrowserTestApp(t)
	now := time.Date(2026, 8, 4, 11, 0, 0, 0, time.UTC)
	node := store.Node{ID: "node-delete", DeviceID: "device-delete", Name: "Delete", Status: "active", AccessMode: "node_auth", CreatedAt: now}
	otherNode := store.Node{ID: "node-keep", DeviceID: "device-keep", Name: "Keep", Status: "active", AccessMode: "node_auth", CreatedAt: now.Add(time.Second)}
	...
	forbiddenRequest.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
	forbidden := httptest.NewRecorder()
...
func TestRelayNodeStoreFailuresReturnInternalError(t *testing.T) {
	...
	listRequest.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
	...
	renameRequest.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
	...
	deleteRequest.AddCookie(&http.Cookie{Name: adminSessionCookie, Value: sessionToken})
...
func (s relayFailureStore) ListNodes(context.Context) ([]store.Node, error) {
	return nil, s.listErr
}

func (s relayFailureStore) RenameNode(context.Context, string, string) (store.Node, error) {
	return store.Node{}, s.renameErr
}

func (s relayFailureStore) DeleteNode(context.Context, string) error {
	return s.deleteErr
}

func browserAdminSession(t *testing.T, application *App) string {
	t.Helper()
	sessionToken, _, err := application.auth.Login(context.Background(), "admin", "secret")
	if err != nil {
		t.Fatal(err)
	}
	return sessionToken
